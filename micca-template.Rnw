...
%Redefine subfloat as per knitr instructions for using
...
#measure_categories <- c("private","medicaid", "other")
@
...
%\begin{tcolorbox}[breakable,colback=white,colframe=cyan,width=\dimexpr\textwidth+4mm\relax,enlarge left by=-2mm]
...
\section*{Women Delivered}
  ...
  \begin{minipage}[c]{0.65\textwidth}
    \strut\vspace*{-\baselineskip}\newline
    << delv_bar, include=TRUE, fig.show='hold', fig.width=6.5, fig.height=3.3, dev='pdf' >>=
    f_wmn_delv_bar
    @
  \end{minipage}
\end{tcolorbox}
...
\noindent\framebox[\textwidth][c]{%
   << counsel_circ, include=TRUE, dev='pdf', out.width='0.2\\textwidth', fig.align='center', fig.show='hold', fig.width=2.5, fig.height=3 >>= 
   f_counsel_circ
   f_document_circ
   @
   
 \begin{minipage}[b]{0.40\textwidth}
   << counsel_bar, include=TRUE, message=FALSE, warning=FALSE, fig.show='hold', fig.width=4, fig.height=3, dev='pdf' >>= 
   f_counsel_doc
   @
 \end{minipage}
}
