...
normalize_umich_data <- function(raw_data){
  raw_data %>% 
    filter(!is.na(mrn))  %>%
...
# Normalize
df_norm <- normalize_umich_data(df_raw)
# Emit normalized data
write_csv(df_norm, "umich_norm_2015-07-01_2020-04-06.csv")

# Calculate components and measures
comp_df <- calculate_components(df_norm)
measure_df <- calculate_measures(comp_df)
