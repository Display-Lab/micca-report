...
calculate_hurley_components <- function(proc_data){
  proc_data %>% 
    ...
    summarize(
      C1 = sum(!is.na(delivery_date)),
      C2 = sum(contra_prov == "0-3 days"),
      C3 = sum(contra_prov == "4-60 days"),
      C4 = sum(larc_prov == "0-3 days"),
      C5 = sum(larc_prov == "4-60 days"),
      C6 = length(unique(mrn)),
      C7 = sum(counseling),
      C8 = sum(contra_choice != "None"),
      C9 = sum(contra_choice == "immediate PP IUD" | contra_choice == "immediate PP Nexplanon"),
      C10 = sum(!is.na(delivery_date) & contra_choice != "None"),
      C11 = sum(!is.na(delivery_date) & counseling & contra_choice == imm_method),
      C12 = sum(!is.na(delivery_date) & larc_prov == "4-60 days"),
